...
class AgricolaAI:
    ...
    def __init__(self, seed=None):
        super(AgricolaAI, self).__init__()
        if seed is not None:
            random.seed(seed)
        ...
        self.observation_space = spaces.Box(low=0, high=np.inf, shape=(len(self.generate_obs(self.player1_state, self.player2_state, self.player3_state, self.player4_state,
                                     self.board_resource, self.reserve_resource, self.tiles)),), dtype=np.float32)
    ...
    def reset(self):
        ...
        self.player1_state = {'round': 0, 'point': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0, 'sheep': 0, 'boar': 0,
                              'cow': 0,
        ...
        self.player2_state = {'round': 0, 'point': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0, 'sheep': 0, 'boar': 0,
                              'cow': 0,
        ...
        self.player3_state = {'round': 0, 'point': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0, 'sheep': 0, 'boar': 0,
                              'cow': 0,
                              'begging': 0, 'rooster': 0, 'clay_conversion': 0, 'clay_room': 0, 'wood_room': 0,
                              'room_space': 2, 'livestock_space': 1, 'farmer': 2,
                              'grain_on_field': 0, 'pasteur_2': 0, 'pasteur_4': 0, 'pasteur_6': 0, 'pasteur_8': 0,
        ...
        self.player4_state = {'round': 0, 'point': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0, 'sheep': 0, 'boar': 0,
                              'cow': 0,
                              'begging': 0, 'rooster': 0, 'clay_conversion': 0, 'clay_room': 0, 'wood_room': 0,
                              'room_space': 2, 'livestock_space': 1, 'farmer': 2,
                              'grain_on_field': 0, 'pasteur_2': 0, 'pasteur_4': 0, 'pasteur_6': 0, 'pasteur_8': 0,
        ...
        self.action_space = self.action_space = spaces.Discrete(len(self.moves))
    ...
    def use_livestock_to_feed(self, player_state, player_missing_foods, player_improvements):
        ...
        print(
            f"{improvement['name']} was used on {feeding_option[0]} sheep, {feeding_option[1]} boar, and {feeding_option[2]} cow to exchange for {acquired_food} food")
            ...
            for order in self.player_order:
                ...
                if order == 'player1':
                    if self.player1_number_actions != 0:
                        print('-----------------------PLAYER 1-----------------------')
                        print(f'action index is {action}')
                        print(f'length of action list is {len(self.moves)}')

                        print('States BEFORE actions')
                        print(self.player1_state)
                        self.player1_action = self.moves[action]  # player 1 takes action from the training agent
...
class AgricolaEnv(gym.Env):
    def __init__(self):
        ...
        # Define action and observation space
        self.action_space = spaces.Discrete(len(self.game.moves))
    ...
    def step(self, action):
        ...
        obs, reward, done, self.action_space = self.game.step(action-1)

        state_delta = obs - self.previous_state
        self.previous_state = obs

        return state_delta, reward, done, {}
