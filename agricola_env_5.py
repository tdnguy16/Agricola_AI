...
class AgricolaAI:
    ...
    def resrouce_check(self):
        if self.board_resource['2_clay'] + self.board_resource['1_clay'] + self.reserve_resource['clay'] + self.player1_state['clay'] + self.player2_state['clay'] + self.player3_state['clay']  + self.player4_state['clay'] != 30:
            sys.exit("Wrong total of clay exists")
        if self.board_resource['reed'] + self.reserve_resource['reed'] + self.player1_state['reed'] + self.player2_state['reed'] + self.player3_state['reed'] + self.player4_state['reed'] != 20:
            sys.exit("Wrong total of reed exists")
        if self.board_resource['1_wood'] + self.board_resource['2_wood'] + self.board_resource['3_wood'] + self.reserve_resource['wood'] + self.player1_state['wood'] + self.player2_state['wood'] + self.player3_state['wood'] + self.player4_state['wood'] != 40:
            sys.exit("Wrong total of wood exists")
        if self.reserve_resource['grain'] + self.player1_state['grain'] + self.player2_state['grain'] + self.player3_state['grain'] + self.player4_state['grain'] + sum(self.player1_field_dict.values()) + sum(self.player2_field_dict.values()) + sum(self.player3_field_dict.values()) + sum(self.player4_field_dict.values()) != 31:
        ...
        if self.board_resource['food'] +  self.reserve_resource['food'] + self.player1_state['food'] + self.player2_state['food'] + self.player3_state['food'] + self.player4_state['food'] != 80:
        ...
        if self.board_resource['sheep'] +  self.reserve_resource['sheep'] + self.player1_state['sheep'] + self.player2_state['sheep'] + self.player3_state['sheep'] + self.player4_state['sheep'] != 26:
        ...
        if self.board_resource['boar'] +  self.reserve_resource['boar'] + self.player1_state['boar'] + self.player2_state['boar'] + self.player3_state['boar'] + self.player4_state['boar'] != 19:
        ...
        if self.board_resource['cow'] +  self.reserve_resource['cow'] + self.player1_state['cow'] + self.player2_state['cow'] + self.player3_state['cow']  + self.player4_state['cow'] != 17:
        ...
        if self.tiles['pasteur_4'] + self.player1_state['pasteur_4'] + self.player2_state['pasteur_4'] + self.player3_state['pasteur_4'] + self.player4_state['pasteur_4'] != 13:
            sys.exit("Wrong total of pasteur_4 exists")
        if self.tiles['pasteur_6'] + self.player1_state['pasteur_6'] + self.player2_state['pasteur_6'] + self.player3_state['pasteur_6'] + self.player4_state['pasteur_6'] != 2:
            sys.exit("Wrong total of pasteur_6 exists")
        if self.tiles['pasteur_8'] + self.player1_state['pasteur_8'] + self.player2_state['pasteur_8'] + self.player3_state['pasteur_8'] + self.player4_state['pasteur_8'] != 1:
            sys.exit("Wrong total of pasteur_8 exists")
        if self.tiles['stable'] + self.player1_state['stable'] + self.player2_state['stable'] + self.player3_state['stable'] + self.player4_state['stable'] != 10:
            sys.exit("Wrong total of stable exists")
        if self.tiles['room'] + self.player1_state['wood_room'] + self.player2_state['wood_room'] + self.player3_state['wood_room'] + self.player4_state['wood_room'] + self.player1_state['clay_room'] + self.player2_state['clay_room'] + self.player3_state['clay_room'] + self.player4_state['clay_room'] != 12:
        ...
        if self.remain_actions > 0:
            for order in self.player_order:
                ...
                if order == 'player1':
                    ...
                    if self.round in (4, 7, 9, 11, 13, 14) and self.player1_number_actions == 0 and self.player_1_harvesting_check[self.round] == 1:
                        # Harvesting:
                        # !!print('=====================================================================================================')
                ...
                if order == 'player2':
                    ...
                    if self.round in (4, 7, 9, 11, 13, 14) and self.player2_number_actions == 0 and self.player_2_harvesting_check[self.round] == 1:
                        # Harvesting:
                ...
                if order == 'player3':
                    ...
                    if self.round in (4, 7, 9, 11, 13, 14) and self.player3_number_actions == 0 and self.player_3_harvesting_check[self.round] == 1:
                        # Harvesting:
                ...
                if order == 'player4':
                    ...
                    if self.round in (4, 7, 9, 11, 13, 14) and self.player4_number_actions == 0 and self.player_4_harvesting_check[self.round] == 1:
                        # Harvesting:
...
if training == True:
    ...
    partner_model = PPO.load(f"E:\\Agricola\\{starting_model}", env=AgricolaEnv())
    ...
    model.save(f"E:\\Agricola\\{model_name}_0")

    average_points_player1 = []
    average_points_player2 = []
    average_points_player3 = []
    average_points_player4 = []

    save_path = f'G:\\My Drive\\Agricola_AI\\Charts\\{model_name}'
    os.makedirs(save_path)

    # Repeat training
    for training_round in range(0, training_rounds, 1):
        total_action_used = []
        agent_action_used = []
        partner_action_used = []
        final_points_player1 = []
        ...
        model = PPO(
            ...
            batch_size=64,
            gamma=0.901510767006798,
            ent_coef=agent_entropy,  # Use the custom entropy coefficient
            learning_rate=agent_learning_rate,  # Use the custom learning rate
            verbose=1
        )

        model = model.load(f"E:\\Agricola\\{model_name}_{training_round}", env=env)

        model.learn(total_timesteps=times_steps)
        model.save(f"E:\\Agricola\\{model_name}_{training_round+1}")

        # Evaluate the Continued Model (optional)
        mean_reward, std_reward = evaluate_policy(model, env, n_eval_episodes=10)
        print(f"------ Training round {training_round+1} ------")
        print(f"Mean reward: {mean_reward} +/- {std_reward}")

        average_points_player1.append(sum(final_points_player1)/len(final_points_player1))
        average_points_player2.append(sum(final_points_player2)/len(final_points_player2))
        average_points_player3.append(sum(final_points_player3)/len(final_points_player3))
        average_points_player4.append(sum(final_points_player4)/len(final_points_player4))

        chart_occurrences(total_action_used, f'{model_name}_Occurrences_of_Actions_total_training round {training_round+1}.png', save_path)
        chart_occurrences(agent_action_used, f'{model_name}_Occurrences_of_Actions_agent_training round {training_round+1}.png', save_path)
        chart_occurrences(partner_action_used, f'{model_name}_Occurrences_of_Actions_partner_training round {training_round+1}.png', save_path)
        ...
        agent_level_occurrences = count_action_levels(moves_total_levels, agent_action_used)
        total_level_occurrences = count_action_levels(moves_total_levels, total_action_used)
        partner_level_occurrences = count_action_levels(moves_total_levels, partner_action_used)

        plot_level_occurrences(agent_level_occurrences, f'{model_name}_Agent_Actions_Levels_training round {training_round + 1}.png', save_path)
        plot_level_occurrences(total_level_occurrences, f'{model_name}_Total_Actions_Levels_training round {training_round + 1}.png', save_path)
        plot_level_occurrences(partner_level_occurrences, f'{model_name}_Partner_Actions_Levels_training round {training_round + 1}.png', save_path)

    plot_points_trend(average_points_player1, average_points_player2, average_points_player3, average_points_player4, name=model_name, save_path=save_path)

    # Dictionary of resource histories for each player
    resource_histories = {
        'player1': resource_player1,
        'player2': resource_player2,
        'player3': resource_player3,
        'player4': resource_player4
    }

    # Plot the average value of clay over rounds
    plot_average_resource_over_rounds(resource_histories, 'clay', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'reed', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'wood', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'grain', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'food', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'sheep', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'boar', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'cow', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'begging', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'rooster', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'clay_room', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'wood_room', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'farmer', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'field', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'pasteur_2', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'pasteur_4', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'pasteur_6', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'pasteur_8', name=model_name, save_path=save_path)
    plot_average_resource_over_rounds(resource_histories, 'stable', name=model_name, save_path=save_path)

    player_improvement_lists = [improvement_player1, improvement_player2, improvement_player3, improvement_player4]
    plot_improvement_occurrences(player_improvement_lists, name=model_name, save_path=save_path)

    plot_action_occurrences_per_round_colored(moves_total_levels, agent_action_round_1, agent_action_round_2, agent_action_round_3, agent_action_round_4, agent_action_round_5, agent_action_round_6, agent_action_round_7, agent_action_round_8, agent_action_round_9, agent_action_round_10, agent_action_round_11, agent_action_round_12, agent_action_round_13, agent_action_round_14, player='Agent', name=model_name, save_path=save_path)
    plot_action_occurrences_per_round_colored(moves_total_levels, partner_action_round_1, partner_action_round_2, partner_action_round_3, partner_action_round_4, partner_action_round_5, partner_action_round_6, partner_action_round_7, partner_action_round_8, partner_action_round_9, partner_action_round_10, partner_action_round_11, partner_action_round_12, partner_action_round_13, partner_action_round_14, player='Partner', name=model_name, save_path=save_path)
...
# Training order: valid_action > animal > feed > Resource > Development > Wood Room > Clay Room > Farmer (2 times)
### General:
# model.ent_coef = 5.467057108921686e-05
# model.learning_rate = 1.1571376087582738e-05
### General 2:
# model.ent_coef = 5.467057108921686e-02
# model.learning_rate = 1.1571376087582738e-05
### General 3:
# Starting model = General_20
# model.ent_coef = 5.467057108921686e-02
# model.learning_rate = 1.1571376087582738e-05
# times_steps = 100000
### General 4:
# Starting model = General_3_20
# partner_model.ent_coef = 5.467057108921686e-01
# model.ent_coef = 5.467057108921686e-02
# model.learning_rate = 1.1571376087582738e-05
# times_steps = 1000000
# 10 rounds (pc errors cut off)
### General 5:
# Starting model = General_4_10
# partner_model.ent_coef = 5.467057108921686e-01
# model.ent_coef = 5.467057108921686e-02
# model.learning_rate = 1.1571376087582738e-05
# times_steps = 1000000
...
# 4 rounds
### General 7:
# Unlike previous training, this time the partner will stay strict to the policy while the training agent explores more
# Starting model = General_6_4
# partner_model.ent_coef = 5.467057108921686e-05
# model.ent_coef = 5.467057108921686e-01
# model.learning_rate = 1.1571376087582738e-05
# times_steps = 1000000
# 4 rounds
### General 8:
# Unlike previous training, this time the agent will stay strict to the policy while the partners explore more
# starting_model = General_7_4
# times_steps = 1000000
# training_rounds = 4
# partner_entropy = 5.467057108921686e-01
# partner_learning_rate = 1.1571376087582738e-05
# agent_entropy = 5.467057108921686e-05
# agent_learning_rate = 1.1571376087582738e-05
### General 9:
# Since model 8 is still gain higher points, doing the same thing for this train
# starting_model = General_8_4
# times_steps = 1000000
# training_rounds = 4
# partner_entropy = 5.467057108921686e-01
# partner_learning_rate = 1.1571376087582738e-05
...
# agent_learning_rate = 1.1571376087582738e-05
### General 10:
# Customized hidden layers of both policy network and value network
# starting_model = 'General_9_4'
# times_steps = 1000000
# training_rounds = 4
# partner_entropy = 5.467057108921686e-01
# partner_learning_rate = 1.1571376087582738e-05
...
### General 7_a
# Keep the agent more creative in find the optimal strategy, it may get lower points at first but hopefully get better after some trainings
# starting_model = 'General_7_4'
# times_steps = 1000000
# training_rounds = 4
# partner_entropy = 5.467057108921686e-05
