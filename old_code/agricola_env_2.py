...
class AgricolaAI:
    ...
    def reset(self):
        ...
        self.player1_state = {'point': 0, 'round': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0,
                              'sheep': 0, 'boar': 0,
                              'cow': 0,
        ...
        self.player2_state = {'point': 0, 'round': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0,
                              'sheep': 0, 'boar': 0,
                              'cow': 0,
        ...
        self.player3_state = {'point': 0, 'round': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0,
                              'sheep': 0, 'boar': 0,
                              'cow': 0,
                              'begging': 0, 'rooster': 0, 'clay_conversion': 0, 'clay_room': 0, 'wood_room': 0,
                              'room_space': 2, 'livestock_space': 1, 'farmer': 2,
        ...
        self.player4_state = {'point': 0, 'round': 0, 'clay': 0, 'reed': 0, 'wood': 0, 'grain': 0, 'food': 0,
                              'sheep': 0, 'boar': 0,
                              'cow': 0,
                              'begging': 0, 'rooster': 0, 'clay_conversion': 0, 'clay_room': 0, 'wood_room': 0,
                              'room_space': 2, 'livestock_space': 1, 'farmer': 2,
        ...
        self.new_round_action_set(self.round)
    ...
    def use_livestock_to_feed(self, player_state, player_missing_foods, player_improvements):
        # Assumption: prioritize the use of livestock for food in this order: cow, boar, sheep
        if len(player_improvements) < 1:
            return player_missing_foods
    ...
    def check_negative_resource(self, data):
        ignore_indices = [0, 25, 50, 75]  # Indices to be ignored
    ...
    def new_round_action_set(self, round):
    ...
    def step(self, action):
            ...
            for order in self.player_order:
                ...
                if order == 'player1':
                    if self.player1_number_actions != 0:
                        ...
                        self.player1_action = self.moves[action[action_index]]  # player 1 takes action from the training agent
        ...
        reward = self.calculate_reward('cow')
        print(reward)
...
class AgricolaEnv(gym.Env):
    ...
    def update_action_space(self, playing_order, p1_actions, p2_actions, p3_actions, p4_actions):
        player_actions = [p1_actions, p2_actions, p3_actions, p4_actions]
        remaining_actions = sum(player_actions)

        x = len(self.game.moves) - 1
        ...
        # #!!print(f'Playing order {playing_order}')
    ...
    def reset(self):
        ...
        self.action_space = spaces.MultiDiscrete(self.action_space_list)
    ...
    def step(self, action):
        ...
        moves = info[0]
        playing_order = info[1]
        player1_number_actions = info[2]
        player2_number_actions = info[3]
        player3_number_actions = info[4]
        player4_number_actions = info[5]

        self.action_space_list = self.update_action_space(playing_order, player1_number_actions, player2_number_actions, player3_number_actions, player4_number_actions)
        self.action_space = spaces.MultiDiscrete(self.action_space_list)
...
# model = PPO.load("ppo_agricola_cow", env=env)
model.learn(total_timesteps=1000000)
model.save("ppo_agricola_cow")

# Evaluate the Continued Model (optional)
mean_reward, std_reward = evaluate_policy(model, env, n_eval_episodes=10)
print(f"Mean reward: {mean_reward} +/- {std_reward}")

# ppo_agricola_best_continued_2: get sheeps
# ppo_agricola_best_continued_3: get sheeps and pasteur_2
# ppo_agricola_best_continued_4: get clays
# ppo_agricola_best_continued_5: get reeds
# ppo_agricola_best_continued_6: get woods
# ppo_agricola_best_continued_7: get more woods (failed)
# ppo_agricola_best_continued_8: get foods (failed)
# ppo_agricola_best_continued_9: get more sheeps
